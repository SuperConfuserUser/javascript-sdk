...
export class Vapp extends Entity implements EntityWithPerfSamples {
   ...
   * @return {Promise<VappBackupStatus>}
   */
  /* istanbul ignore next: autogenerated */
  async getBackupStatus(): Promise<VappBackupStatus> {
    return Iland.getHttp().get(`/vapps/${this.uuid}/backup-status`).then((response) => {
      const json = response.data as VappBackupStatusJson;
      return new VappBackupStatus(json);
    });
  }
}
