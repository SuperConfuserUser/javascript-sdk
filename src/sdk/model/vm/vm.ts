...
import { VmBackupStatus } from '../advanced-backups/backup-status/vm-backup-status';
import { VmBackupStatusJson } from '../advanced-backups/backup-status/__json__/vm-backup-status-json';
...
export class Vm extends Entity implements EntityWithPerfSamples {
   ...
   *
   * @return {Promise<VmBackupStatus>}
   */
  /* istanbul ignore next: autogenerated */
  async getBackupStatus(): Promise<VmBackupStatus> {
    return Iland.getHttp().get(`/vms/${this.uuid}/backup-status`).then((response) => {
      const json = response.data as VmBackupStatusJson;
      return new VmBackupStatus(json);
    });
  }
   ...
   */
  async restoreVMBackupsInVdc(params: RestoreVmBackupParams): Promise<Task> {
   ...
   * the VM.
   *
   * @param {SearchVmRecoverableFilesAndFoldersFilters} filters optional query filters
   ...
   * @param {number} attemptNumber the snapshot attempt number
   ...
   * @param {ListBackupSnapshotFilesAndFoldersFilters} filters optional query filters // fixme: is the whole thing
   *   optional or each prop is optional?
